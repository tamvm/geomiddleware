 ...
 */
@Repository
public class OpenStreetMapDao extends GenericDao<OpenStreetMap, Long> implements
		IOpenStreetMapDao {
	@Autowired
	...
	public List<StreetDistance> getNearestAndDistanceFrom(final Point point,
			final double distance, final int firstResult, final int maxResults,
			final StreetType streetType, final Boolean oneWay,
			final String name, final StreetSearchMode streetSearchMode,
			final boolean includeDistanceField) {

		Assert.notNull(point);
		if (name != null && streetSearchMode == null) {
			throw new IllegalArgumentException(
					"streetSearchmode can not be null if name is provided");
		}
		return (List<StreetDistance>) this.getHT().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
						...
						if (includeDistanceField) {
							projections.add(
							// SpatialProjection.distance_sphere(point,
									// GisFeature.LOCATION_COLUMN_NAME).as(
									// "distance"));
									SpatialProjection.distance_pointToLine(
											point,
											OpenStreetMap.SHAPE_COLUMN_NAME)
											.as("distance"));
						}
						...
						Polygon polygonBox = GeolocHelper.createPolygonBox(
								point.getX(), point.getY(), distance);
						criteria = criteria.add(new IntersectsRestriction(
								OpenStreetMap.SHAPE_COLUMN_NAME, polygonBox));
						if (!StaticFunc.isNOE(name)) {
							if (streetSearchMode == StreetSearchMode.CONTAINS) {
								criteria = criteria.add(Restrictions
										.isNotNull("name"));// optimisation!
								criteria = criteria
										.add(Restrictions
												.ilike(
														OpenStreetMap.FULLTEXTSEARCH_PROPERTY_NAME,
														"%" + name + "%"));
								// criteria = criteria.add(new
								// PartialWordSearchRestriction(OpenStreetMap.PARTIALSEARCH_VECTOR_COLUMN_NAME,
								// name));
							} else if (streetSearchMode == StreetSearchMode.FULLTEXT) {
								criteria = criteria
										.add(new FulltextRestriction(
												OpenStreetMap.FULLTEXTSEARCH_VECTOR_PROPERTY_NAME,
												name));
							} else {
								throw new NotImplementedException(
										streetSearchMode
												+ " is not implemented for street search");
							}
						}
						...
						if (oneWay != null) {
							criteria = criteria.add(Restrictions.eq("oneWay",
									oneWay));
						}
						...
						if (queryResults != null && queryResults.size() != 0) {
							...
							List<StreetDistance> results = ResultTransformerUtil
									.transformToStreetDistance(
											propertiesNameArray, queryResults);
							return results;
						} else {
	...
	public OpenStreetMap getByGid(final Long gid) {
		Assert.notNull(gid);
		return (OpenStreetMap) this.getHT().execute(new HibernateCallback() {
	 ...
	 * (non-Javadoc)
	 * 
	 * @seecom.gisgraphy.domain.repository.IOpenStreetMapDao#
	 * buildIndexForStreetNameSearch()
	 */
	public Integer updateTS_vectorColumnForStreetNameSearch() {
		return (Integer) this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
				...
				String updateFulltextField = "UPDATE openStreetMap SET "
						...
						+ OpenStreetMap.FULLTEXTSEARCH_COLUMN_NAME
						+ ",'')) where name is not null";
				Query qryUpdateFulltextField = session
						.createSQLQuery(updateFulltextField);
				int numberOfLineUpdatedForFulltext = qryUpdateFulltextField
						.executeUpdate();
				int numberOfLineUpdatedForPartial = 0;
				if (GisgraphyConfig.PARTIAL_SEARH_EXPERIMENTAL) {
					logger.info("will update "
							+ OpenStreetMap.PARTIALSEARCH_VECTOR_PROPERTY_NAME
									.toLowerCase() + " field");
					String updatePartialWordField = "UPDATE openStreetMap SET "
							+ OpenStreetMap.PARTIALSEARCH_VECTOR_PROPERTY_NAME
									.toLowerCase()
							+ " = to_tsvector('simple',coalesce("
							+ OpenStreetMap.PARTIALSEARCH_COLUMN_NAME
							+ " ,'')) where name is not null";
					Query qryUpdateParialWordField = session
							.createSQLQuery(updatePartialWordField);
					numberOfLineUpdatedForPartial = qryUpdateParialWordField
							.executeUpdate();
					session.flush();
				}
				return Integer.valueOf(numberOfLineUpdatedForFulltext
	 ...
	 * buildIndexForStreetNameSearch()
	 */
	public Integer updateTS_vectorColumnForStreetNameSearchPaginate(
			...
			public Object doInHibernate(Session session)
				...
				if (GisgraphyConfig.PARTIAL_SEARH_EXPERIMENTAL) {
					...
					String updatePartialWordField = "UPDATE openStreetMap SET "
							...
							+ " = to_tsvector('simple',coalesce("
							+ OpenStreetMap.PARTIALSEARCH_COLUMN_NAME
							+ " ,'')) where gid >= " + from + " and gid <= "
							+ to + " and name is not null";
					Query qryUpdateParialWordField = session
							.createSQLQuery(updatePartialWordField);
					numberOfLineUpdatedForPartial = qryUpdateParialWordField
							.executeUpdate();
	...
	public void createSpatialIndexes() {
		this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
					throws PersistenceException {
				session.flush();
				logger.info("will create GIST index for  the "
						+ OpenStreetMap.SHAPE_COLUMN_NAME + " column");
				String createIndexForShape = "CREATE INDEX "
						+ OpenStreetMap.SHAPE_COLUMN_NAME.toLowerCase()
						+ "indexopenstreetmap ON openstreetmap USING GIST ("
						+ OpenStreetMap.SHAPE_COLUMN_NAME.toLowerCase() + ")";
				Query qryUpdateFulltextField = session
						.createSQLQuery(createIndexForShape);
		...
		this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
				...
				String createFulltextIndex = "CREATE INDEX "
						...
						+ OpenStreetMap.FULLTEXTSEARCH_VECTOR_PROPERTY_NAME
								.toLowerCase() + ")";
				Query fulltextIndexQuery = session
						.createSQLQuery(createFulltextIndex);
	...
	public void clearPartialSearchName() {
		this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
				...
				String clearTextSearchNameQueryString = "Update openstreetmap set  "
						+ OpenStreetMap.PARTIALSEARCH_COLUMN_NAME.toLowerCase()
						+ "= null";
				Query fulltextIndexQuery = session
						.createSQLQuery(clearTextSearchNameQueryString);
				fulltextIndexQuery.executeUpdate();
