...
public class StreetSearchEngine implements IStreetSearchEngine {
	 ...
	 * @author VoMinhTam
	 */
	public StreetSearchEngine(IOpenStreetMapDao openStreetMapDao,
			IStreetSearchResultsDtoSerializer streetSearchResultsDtoSerializer,
			IStatsUsageService statsUsageService) {
		this.openStreetMapDao = openStreetMapDao;
		this.streetSearchResultsDtoSerializer = streetSearchResultsDtoSerializer;
		...
		List<StreetDistance> results = openStreetMapDao
				.getNearestAndDistanceFrom(query.getPoint(), query.getRadius(),
						...
						query.getStreetType(), query.getOneWay(), query
								.getName(), query.getStreetSearchMode(), query
								.hasDistanceField());
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.geoloc.service.IQueryProcessor#executeAndSerialize
	 * (com.gisgraphy.domain.geoloc.service.AbstractGisQuery,
	 * java.io.OutputStream)
	 */
	public void executeAndSerialize(StreetSearchQuery query,
			OutputStream outputStream) throws ServiceException {
		Assert.notNull(query, "Can not execute a null query");
		...
		extraParameter
				.put(
						StreetSearchResultsDtoSerializer.START_PAGINATION_INDEX_EXTRA_PARAMETER,
						query.getFirstPaginationIndex());
		extraParameter.put(UniversalSerializerConstant.CALLBACK_METHOD_NAME,
				query.getCallback());
		streetSearchResultsDtoSerializer.serialize(outputStream, query
				.getOutputFormat(), streetSearchResultsDto, query
				.isOutputIndented(), extraParameter);
	}
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.geoloc.service.IQueryProcessor#executeQueryToString
	 * (com.gisgraphy.domain.geoloc.service.AbstractGisQuery)
	 */
