...
public class FulltextQueryHttpBuilder {
	...
	public FulltextQuery buildFromRequest(HttpServletRequest req) {
		...
		String httpQueryParameter = req
				.getParameter(FulltextServlet.QUERY_PARAMETER);
		...
		pagination = Pagination.paginateWithMaxResults(
				query.getMaxLimitResult()).from(from).to(to)
		...
		} else if ("false".equalsIgnoreCase(req
				.getParameter(FulltextServlet.SPELLCHECKING_PARAMETER))) {
