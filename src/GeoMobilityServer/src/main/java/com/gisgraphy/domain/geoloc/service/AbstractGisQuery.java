...
/**
 * An abstract query for all GisQuery. define {@link Output}, {@link Pagination}
 * , and a placetype
 * 
...
public abstract class AbstractGisQuery {
	 ...
	 *            default
	 * @param output
	 *            {@linkplain Output} The output specification , if null : use
	 *            default
	 */
	...
	/**
	 * @param output
	 *            The {@link Output} Object to set. If the output is null : the
	 *            {@link Output#DEFAULT_OUTPUT} is used
	...
	/**
	 * @param callback
	 *            the callback function name, it is use for script language
	 *            (python, json, ruby, ...) the result will be wrap with
	 *            callback(DATA); the callback should be alphanumeric, if not it
	 *            won't be set
	 * @return the current object to chain setters
	 */
	public AbstractGisQuery withCallback(String callback) {
		if (callback != null
				&& callbackValidationPattern.matcher(callback).matches()) {
			this.callback = callback;
		} else {
			logger.warn("wrong callback specify : " + callback
					+ ", callback method sould be alphanumeric");
		}
		return this;
	}
	...
	public String toString() {
		String asString = this.getClass().getSimpleName() + " with "
				+ getOutput() + " and " + pagination + " and callback="
				+ this.callback;
		return asString;
	}
