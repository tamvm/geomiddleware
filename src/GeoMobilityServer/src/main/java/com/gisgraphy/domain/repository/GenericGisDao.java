...
import com.gisgraphy.hibernate.criterion.DistanceRestrictionGreater;
...
public class GenericGisDao<T extends GisFeature> extends
	...
	public static final String TAG = GenericGisDao.class.getSimpleName();
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getNearestAndDistanceFrom(com
	 * .gisgraphy.domain.geoloc.entity.GisFeature, double, int, int)
	 */
	public List<GisFeatureDistance> getNearestAndDistanceFromGisFeature(final GisFeature gisFeature, 
		...
		return getNearestAndDistanceFrom(gisFeature.getLocation(), gisFeature.getId(), distance, firstResult, maxResults,
				includeDistanceField, persistentClass, null, true, 0);
	}
	 ...
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getNearestAndDistanceFrom(com
	 * .gisgraphy.domain.geoloc.entity.GisFeature, double)
	 */
	public List<GisFeatureDistance> getNearestAndDistanceFromGisFeature(
		...
		return getNearestAndDistanceFrom(gisFeature.getLocation(), gisFeature.getId(), 
				distance, -1, -1, includeDistanceField, persistentClass, null,true, 0);
	}
	 ...
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getNearestAndDistanceFrom(com
	 * .vividsolutions.jts.geom.Point, double)
	 */
	public List<GisFeatureDistance> getNearestAndDistanceFrom(Point point, double distance) {
		return getNearestAndDistanceFrom(point, 0L, distance, -1, -1, true, persistentClass, null, true, 0);
	}
	 ...
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getNearestAndDistanceFrom(com
	 * .vividsolutions.jts.geom.Point, double, int, int)
	 */
	public List<GisFeatureDistance> getNearestAndDistanceFrom(Point point,
		...
		return getNearestAndDistanceFrom(point, 0L, distance, firstResult,
				maxResults, includeDistanceField, persistentClass, null, true, 0);
	}

	public List<GisFeatureDistance> getNearestAndDistanceFrom(Point point, double distance, int firstResult, int maxResults, 
			boolean includeDistanceField, String query, double minRadius) {
		return getNearestAndDistanceFrom(point, 0L, distance, firstResult,
				maxResults, includeDistanceField, persistentClass, query, true, minRadius);
	}

	/**
	 * @author <a href="mailto:vo.mita.ov@gmail.com">VoMinhTam</a>
	 */
	public List<GisFeatureDistance> getInPolygon(Polygon polygon, Polygon polygonExterior, int firstResult, int maxResults, 
			final Class<? extends GisFeature> requiredClass, String query, String sortBy) {
		...
		if (polygon!=null){
			...
			criteria = criteria.add(new IntersectsRestriction("location", polygon));
		}
		
		if (polygonExterior!=null){
			Log.d(TAG, polygonExterior.toString());
			IntersectsRestriction notIntersects = new IntersectsRestriction("location", polygonExterior);
			notIntersects.isNOT(true);
			criteria = criteria.add(notIntersects);
		}
		
		// Tam
		if (!StaticFunc.isNOE(query)) {
			criteria = criteria.add(Restrictions.or(Restrictions.sqlRestriction("no_sign(name) ilike '%" + StaticFunc.removeSign(query)+ "%'"), 
					Restrictions.sqlRestriction("no_sign(adm1name) ilike '%" + StaticFunc.removeSign(query)+ "%'")));
		}
		...
		boolean hasZipCodesProperty = ZipCodesAware.class
		...
		List<GisFeatureDistance> results = ResultTransformerUtil
				.transformToGisFeatureDistance(aliasList, queryResults, idToZipCodesMap);

		if (queryResults != null) {
			return (List<GisFeatureDistance>) results;
		} else {
			return new ArrayList<GisFeatureDistance>();
		}
	}

//	protected List<GisFeatureDistance> getNearestAndDistanceFrom(final Point point, final Long pointId, 
//			final double distance, final int firstResult, final int maxResults, boolean includeDistanceField,
//			final Class<? extends GisFeature> requiredClass, String query) {
//		return getNearestAndDistanceFrom(point, pointId, distance, firstResult,
//				maxResults, includeDistanceField, requiredClass, query, true, 0);
//	}
...
//		return getNearestAndDistanceFrom(point, pointId, distance, firstResult,
//				maxResults, includeDistanceField, requiredClass, null, true, 0);
//	}
	 ...
	 * @param pointId
	 *            the id of the point that we don't want to be include, it is
	 *            used to not include the gisFeature from which we want to find
	 *            the nearest
	 * @param firstResult
	 *            the firstResult index (for pagination), numbered from 1, if <
	 *            1 : it will not be taken into account
	 * @param maxResults
	 *            The Maximum number of results to retrieve (for pagination), if
	 *            <= 0 : it will not be taken into acount
	 * @param requiredClass
	...
	protected List<GisFeatureDistance> getNearestAndDistanceFrom(final Point point, final Long pointId, final double distance, 
			final int firstResult, final int maxResults, boolean includeDistanceField, final Class<? extends GisFeature> requiredClass, 
			String query, boolean findMin, double minRadius) {
		Session session = HibernateUtil.getSession();
		Criteria criteria = session.createCriteria(requiredClass);
...
//			else
		...
		if (minRadius>0)
			criteria = criteria.add(new DistanceRestrictionGreater(point, minRadius));
		
		List<String> fieldList = IntrospectionHelper .getFieldsAsList(requiredClass);
		ProjectionList projections = ProjectionBean.fieldList(fieldList, true);
		if (includeDistanceField) {
			Log.d(TAG, point.toString());
			projections.add(SpatialProjection.distance_sphere(point, GisFeature.LOCATION_COLUMN_NAME).as("distance"));
		}
		// Tam
		if (!StaticFunc.isNOE(query)) {
			criteria = criteria.add(Restrictions.or(Restrictions.sqlRestriction("no_sign(name) ilike '%" + StaticFunc.removeSign(query)+ "%'"), 
		...
		String[] aliasList;
		if (includeDistanceField) {
			aliasList = (String[]) ArrayUtils.add(IntrospectionHelper.getFieldsAsArray(requiredClass), "distance");
		} else {
			aliasList = IntrospectionHelper.getFieldsAsArray(requiredClass);
		}
		int idPropertyIndexInAliasList = 0;
		...
		List<GisFeatureDistance> results = ResultTransformerUtil.transformToGisFeatureDistance(
				aliasList, queryResults,idToZipCodesMap);
		return results;
	}

	/*
	 * (non-Javadoc) Original
	 * 
	...
	public List<T> listByName(final String name) {
		Assert.notNull(name);
		return (List<T>) this.getHT().execute(new HibernateCallback() {

			public Object doInHibernate(Session session)
					throws PersistenceException {
				String queryString = "from " + persistentClass.getSimpleName()
						+ " as c where no_sign(c.name) ilike '%" + StaticFunc.removeSign(name) +"%'";
	...
	@Deprecated
	@SuppressWarnings("unchecked")
	public List<T> listByName(final String query, final boolean isMatchAll,
			final String countryCode, final String sortField, final boolean isAsc) {
		...
		return (List<T>) this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
					throws PersistenceException {
				String nameTmp = query;
				if (!isMatchAll)
					nameTmp = "%" + StaticFunc.removeSign(query) + "%";

				String queryString = "from " + persistentClass.getSimpleName()
						+ " as c where no_sign(c.name) " + ((isMatchAll) ? " = " : " ilike ") + "'" + nameTmp
						+ "' or no_sign(c.adm1Name) ilike '" + nameTmp + "'";
				String whereCountryCode = StaticFunc.isNOE(countryCode) ? "" : " and countrycode = '" + countryCode + "' ";
				String orderBy = StaticFunc.isNOE(sortField) ? "" : " order by " + sortField + ((isAsc) ? " asc " : " desc ");
				queryString += whereCountryCode + orderBy;
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getByFeatureId(java.lang.Long)
	 */
	@SuppressWarnings("unchecked")
	public T getByFeatureId(final Long featureId) {
		Assert.notNull(featureId);
		return (T) this.getHT().execute(new HibernateCallback() {
	...
	public List<T> getDirties() {
		return (List<T>) this.getHT().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session)
					throws PersistenceException {
				final String queryString = "from " + persistentClass.getSimpleName() + " as g where g.featureCode ='"
						+ ImporterConfig.DEFAULT_FEATURE_CODE + "' or g.location=? or g.featureClass='"
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getByFeatureIds(java.util.List)
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#getFromText(java.lang.String,
	 * boolean)
	 */
	...
	public T getEager(final Long id) {
		...
		try {
			...
			logger.info("could not retrieve object of type " + persistentClass.getSimpleName() + " with id " + id, e);
		}
		return returnValue;
	}
	 ...
	 * 
	 * @see
	 * com.gisgraphy.domain.repository.IGisDao#createGISTIndexForLocationColumn
	 * ()
	 */
	public void createGISTIndexForLocationColumn() {
		this.getHT().execute(new HibernateCallback() {

			public Object doInHibernate(Session session)
					throws PersistenceException {
				session.flush();
				logger.info("will create GIST index for  " + persistentClass.getSimpleName());
				String createIndex = "CREATE INDEX locationIndex" + persistentClass.getSimpleName() + 
						" ON " + persistentClass.getSimpleName().toLowerCase() + " USING GIST (location)";
