...
public class StreetSearchResultsDtoSerializer implements
	...
	public void serialize(OutputStream outputStream, OutputFormat outputFormat,
		...
		if (!OutputFormatHelper.isFormatSupported(outputFormat,
			...
			try {
				serializeWithUniveraslSerializer(outputStream,
						streetSearchResultsDto, indent, outputFormat,
						extraParameters);
			...
			serializeToFeed(outputStream, streetSearchResultsDto,
					OutputFormat.ATOM_VERSION, startPaginationIndex);
		} else if (outputFormat == OutputFormat.GEORSS) {
			int startPaginationIndex = getStartPaginationIndex(extraParameters);
			serializeToFeed(outputStream, streetSearchResultsDto,
					OutputFormat.RSS_VERSION, startPaginationIndex);
		} else {
			// default
			try {
	...
	private void serializeWithUniveraslSerializer(OutputStream outputStream,StreetSearchResultsDto streetSearchResultsDto, 
			boolean indent, OutputFormat format, Map<String, Object> extraParameters) throws ServiceException {
		try {
	...
	private void serializeToFeed(OutputStream outputStream,
			StreetSearchResultsDto streetSearchResultsDto, String feedVersion,
		...
		try {
			...
			for (StreetDistance gisFeatureDistance : streetSearchResultsDto
				...
				openSearchModule.setTotalResults(streetSearchResultsDto
						.getNumFound());
				...
				entry.setLink(Constants.STREET_BASE_URL
			...
			try {
				...
				throw new RuntimeException("unknow encoding "
						+ Constants.CHARSET);
			}
