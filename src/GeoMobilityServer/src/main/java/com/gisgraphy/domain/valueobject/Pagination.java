...
public class Pagination {
    ...
    /**
     * the default 'to' parameters if the one specified is missing or
     * incorrect
     */
    ...
    public Pagination(int from, int maxResult){
    	this.maxResult = maxResult;
    	if (maxResult<0)
    		this.maxResult = DEFAULT_MAX_RESULTS;
    	this.from = from;

    	if (this.from < 0)
    		this.from = 0;
    	this.to = this.from + this.maxResult -1;
    }
