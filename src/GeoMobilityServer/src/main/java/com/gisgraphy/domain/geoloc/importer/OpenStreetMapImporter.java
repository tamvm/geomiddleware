...
public class OpenStreetMapImporter extends AbstractImporterProcessor {
	...
	protected void setup() {
		...
		statusMessage = internationalisationService
				.getString("import.message.createIndex");
	...
	protected void processData(String line) throws ImporterException {
		...
		if (!isEmptyField(fields, 2, false)) {
			try {
				Point location = (Point) GeolocHelper
						.convertFromHEXEWKBToGeometry(fields[2]);
				street.setLocation(location);
			} catch (RuntimeException e) {
				logger.warn("can not parse location for " + fields[1] + " : "
						+ e);
			}
		}
		...
		if (!isEmptyField(fields, 6, false)) {
			...
			try {
				...
				logger.warn("can not determine streetType for " + fields[1]
						+ " : " + e);
			}
		...
		if (!isEmptyField(fields, 8, true)) {
			try {
				street.setShape((LineString) GeolocHelper
						.convertFromHEXEWKBToGeometry(fields[8]));
			} catch (RuntimeException e) {
	...
	public List<NameValueDTO<Integer>> rollback() {
		...
		if (deleted != 0) {
			deletedObjectInfo.add(new NameValueDTO<Integer>(openStreetMapDao
					.getPersistenceClass().getSimpleName(), deleted));
		}
	...
	protected void tearDown() {
		...
		this.statusMessage = internationalisationService
				.getString("import.openstreetmap.cleanDatabase");
		try {
