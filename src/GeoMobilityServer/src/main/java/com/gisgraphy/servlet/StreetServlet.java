...
public class StreetServlet extends GisgraphyServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		// check empty query
		if (HTMLHelper.isParametersEmpty(req, LAT_PARAMETER, LONG_PARAMETER)) {
			sendCustomError(ResourceBundle.getBundle(Constants.BUNDLE_ERROR_KEY).getString("error.emptyLatLong"), 
					format, resp, req);
			return;
		}
		StreetSearchQuery query;
		try {
			query = StreetSearchQueryHttpBuilder.getInstance().buildFromHttpRequest(req);
			...
			if (logger.isInfoEnabled()) {
				logger.info("A street request from " + req.getRemoteHost()
						+ " / " + req.getRemoteAddr()
						+ " was received , Referer : " + referer + " , UA : "
						+ UA);
			}

			streetSearchEngine.executeAndSerialize(query, resp.getOutputStream());
		} catch (GeolocSearchException e1) {
			e1.printStackTrace();
		} catch (StreetSearchException e1) {
			e1.printStackTrace();
